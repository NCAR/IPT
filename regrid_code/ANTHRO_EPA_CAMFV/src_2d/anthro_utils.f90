      ...
      subroutine mapper( nemis, emis_map, sub_categories, data_file )
!------------------------------------------------------------------
...
!------------------------------------------------------------------
!     allocate data_file array
!------------------------------------------------------------------
      allocate( data_file(nsrc_names),stat=astat )
      if( astat /= 0 ) then
        write(*,*) 'anthro_utils: failed to allocate data_file type'
        stop 'Alloc err'
      endif
!------------------------------------------------------------------
!     check if src is epa
!------------------------------------------------------------------
      do n = 1,nsrc_names
        ...
        if( sbl > 1 ) then
          sbu = len_trim( src_names(n) )
          if( sbu > sbl ) then
            call upcase( src_names(n)(sbl+1:sbu),wrkStr )
            if( trim(wrkStr) == 'EPA-SECTOR' .or. trim(wrkStr) == 'EPA-STACK') then
              data_file(n)%is_EPA = .true.
              data_file(n)%is_EPA_SECTOR = trim(wrkStr) == 'EPA-SECTOR'
              data_file(n)%is_EPA_STACK  = .not. data_file(n)%is_EPA_SECTOR
            endif
          endif
          src_names(n)(sbl:) = ' '
        else
          data_file(n)%is_EPA = .false.
          data_file(n)%is_EPA_sector = .false.
          data_file(n)%is_EPA_stack  = .false.
        endif
      end do
!------------------------------------------------------------------
...
!------------------------------------------------------------------
   ...
   if( allocated( grid%xedge_2d ) ) then
     deallocate( grid%xedge_2d, grid%yedge_2d, grid%xedge_2d_ij, grid%yedge_2d_ij )
   endif
   if( allocated( grid%lon ) ) then
     deallocate( grid%lon, grid%lat, grid%xedge, grid%yedge )
   endif
   if( allocated( grid%lon_2d ) ) then
     deallocate( grid%lon_2d, grid%lat_2d )
   endif
   if( allocated( grid%z_at_w ) ) then
     deallocate( grid%z_at_w )
   endif
   if( allocated( grid%landmask ) ) then
     deallocate( grid%landmask )
   endif
