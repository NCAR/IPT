...
xfaces = xnodes(faces(:,1:4)+1);
yfaces = ynodes(faces(:,1:4)+1);
zfaces = znodes(faces(:,1:4)+1);

for i = 1:size(xfaces,1)
    total = faces(i,5);
    if (total == -1)
        patch(xfaces(i,:), yfaces(i,:), zfaces(i,:),'w','EdgeColor','none');
    else
        patch(xfaces(i,:), yfaces(i,:), zfaces(i,:),'w','EdgeColor','none','FaceColor',1-total/5*[1 1 1]);
    end
