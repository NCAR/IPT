...
//=============================================================================
void RefinementTemplateLOWCONN::ApplyStandardTemplate( FaceVector & vecFaces    ,
                                                       NodeVector & vecNodes    ,
                                                       int          ixActiveFace,
                                                       int          ixActiveSeg ) 
{
...
} // RefinementTemplateLOWCONN::ApplyStandardTemplate
//=============================================================================


//=============================================================================
void RefinementTemplateLOWCONN::ApplyInnerCornerTemplate(FaceVector & vecFaces    ,
                                                         NodeVector & vecNodes    ,
                                                         int          ixActiveFace,
                                                         int          ixActiveSeg ) 
{
...
/*
    ...
    //---------------------------------------------
    int ixFarNode = InsertQuadNode(vecNodes,
                                   vecNodes[face[(ixActiveSeg+1)%4]],
    ...
    vecNodes[iterSegRight->second.ixMidpointNode[1]].x = vecNodes[ixFarNode].x;
    vecNodes[iterSegRight->second.ixMidpointNode[1]].y = vecNodes[ixFarNode].y;
    vecNodes[iterSegRight->second.ixMidpointNode[1]].z = vecNodes[ixFarNode].z;

    vecNodes.erase(vecNodes.begin() + ixFarNode);

    // Merge opposite node into right node
    MergeNodes( vecNodes, vecFaces,  iterSegOpp->second.ixMidpointNode[1],
                                   iterSegRight->second.ixMidpointNode[1]);

    iterSegOpp->second.ixMidpointNode[1] = iterSegRight->second.ixMidpointNode[1];
*/
/*
    printf("%i %i\n", iterSegOpp->second.ixMidpointNode[1], vecNodes.size());
...
*/
    ...
    // Construct connecting face
    //---------------------------
    if(iFirstConnectingNode == InvalidNode) { iFirstConnectingNode = ixNode2; }
    ...
    if((ixNode2 < 8) || (face[(ixActiveSeg+3)%4] < 8)) {
      ...
      //----------------------------------------------------------------
    } else if((iter0 != mapMergeActive.end()) || (iter1 != mapMergeActive.end())) {
      iter0->second = false;
      iter1->second = false;
    } else {
      // Add new entry to merge structure
      //-----------------------------------
      mapMergeActive.insert(ActiveMergeMapElement(ixMergeNode0, true));
      mapMergeActive.insert(ActiveMergeMapElement(ixMergeNode1, true));
...
} // RefinementTemplateLOWCONN::ApplyInnerCornerTemplate
//=============================================================================


//=============================================================================
void RefinementTemplateLOWCONN::ApplyOuterCornerTemplate(FaceVector & vecFaces    ,
                                                         NodeVector & vecNodes    ,
                                                         int          ixActiveFace,
                                                         int          ixActiveSeg ) 
{
...
} // RefinementTemplateLOWCONN::ApplyOuterCornerTemplate
//=============================================================================
...
//=============================================================================
void RefinementTemplateLOWCONN::RefineLocalGrid( int                 nRefineLevel  ,
                                                FaceVector         & vecFaces      ,
                                                NodeVector         & vecNodes      ,
                                                bool                 fFirstSegment ,
                                                bool                 fLastSegment  ,
                                                ...
                                                SegmentMapIterator & iterSegCurr   )
{
    ...
    if(fLastSegment) {
      ...
      for (iter = setMergePairs.begin(); iter != setMergePairs.end(); iter++) {
        ...
        if((iter0 == mapMergeActive.end()) || (iter1 == mapMergeActive.end())) {
          _EXCEPTIONT("Logic error");
        }
    
        if((iter0->second == false) || (iter1->second == false)) {
          continue;
        }
    
        // Merge node ixMergeNode1 into ixMergeNode0
        //---------------------------------------------
        MergeNodes( vecNodes, vecFaces, ixMergeNode0, ixMergeNode1);
...
} // RefinementTemplateLOWCONN::RefineLocalGrid
//=============================================================================


//=============================================================================
void RefinementTemplateLOWCONN::FinalizeAll(FaceVector & vecFaces, 
                                            NodeVector & vecNodes) 
{
    // Delete nodes
    for (unsigned int i = 0; i < vecSpuriousNodes.size(); i++) {
      int ixDeleteNode = vecSpuriousNodes[i];
      for (unsigned int j = i+1; j < vecSpuriousNodes.size(); j++) {
        if(vecSpuriousNodes[j] > ixDeleteNode) {
          vecSpuriousNodes[j]--;
        }
      }
      
      for (unsigned int j = 0; j < vecFaces.size(); j++) {
      for (         int c = 0; c < 4              ; c++) {
        if(vecFaces[j][c] == ixDeleteNode) { _EXCEPTION();     }
        if(vecFaces[j][c] >  ixDeleteNode) { vecFaces[j][c]--; }
      }
      }
...
} // RefinementTemplateLOWCONN::FinalizeAll
//=============================================================================
