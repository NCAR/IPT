...
//=============================================================================
void Tessellate( NodeVector & vecNodes, FaceVector & vecFaces) 
{
    ...
    for (; iter != mapSegment.end(); iter++) {
      Face faceNew = Face(iter->first[1]                        ,
...
//=============================================================================
void RefineEverything( NodeVector & vecNodes, FaceVector & vecFaces, int nResolution) 
{
    ...
    for (; iter != mapSegment.end(); iter++) {
      Edge edge;
      GenerateEdgeVertices( nResolution, iter->first[0], iter->first[1], vecNodes, edge);
      mapEdge.insert(std::pair<Segment, Edge>(iter->first, edge));
    }
    
    // Loop over all faces and refine
    //----------------------------------
    for (unsigned int n = 0 ; n < vecFacesOld.size(); n++) {
