...
##--------------------------------------------------------
  ...
  echo " "
  echo "  #--------------------------------------------------------- "
  echo "  # Creating gen_domains scripts in directory: "  $GRID_NAME
  echo "  # "
  echo "  #  Review the variable settings at the begining of the "
  echo "  #  scripts and then run them:"
  echo "  # "
  echo "  #   cheyenne% qcmd -- 'sh genDomains_"$GRID_TAG"_gx1v7.sh   >& LOG_"$GRID_TAG"_gx1v7'"
  echo "  #   cheyenne% qcmd -- 'sh genDomains_"$GRID_TAG"_tx0.1v2.sh >& LOG_"$GRID_TAG"_tx0.1v2'"
  echo "  # "
  echo "  #--------------------------------------------------------- "
  echo " "
  mkdir -p $GRID_NAME
  sed -e $SEDcmdA -e $SEDcmdT -e $SEDcmdR -e $SEDcmdG -e $SEDcmdS -e $SEDcmdD < ./TEMPLATES/genDomains_TEMPLATE_gx1v7.sh   >  ./$GRID_NAME/genDomains_${GRID_TAG}_gx1v7.sh
  sed -e $SEDcmdA -e $SEDcmdT -e $SEDcmdR -e $SEDcmdG -e $SEDcmdS -e $SEDcmdD < ./TEMPLATES/genDomains_TEMPLATE_tx0.1v2.sh >  ./$GRID_NAME/genDomains_${GRID_TAG}_tx0.1v2.sh
