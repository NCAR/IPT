...
void RefinementTemplateCUBIT::RefineLocalGrid(
	int nRefineLevel,
	FaceVector & vecFaces,
	NodeVector & vecNodes,
	bool fFirstSegment,
	...
	if ((faceActive.ContainsSegment(segNext) != (-1)) && (fLastSegment)) {
		return;
	}
	...
	// Apply standard refinement pattern (full edge previous)
		...
		int ixCentroidNode =
			AddFaceCenterpointNode(faceActive, vecNodes);

		// Bisect opposite face
		int ixOppSeg =
			faceActive.ContainsSegment(iterSegCurr->first);
		...
		SegmentMapIterator & iterSegActive = 
			faceActive.iterSegment[(ixOppSeg + 3) % 4];
		...
		vecFaces.push_back(Face(
			iterSegCurr->second.ixMidpointNode[0],
			ixCentroidNode,
			ixBisectNode,
			ixCommonNode));
	...
	if ((faceInactive.ContainsSegment(segNext) != (-1)) &&
		...
		const Segment & segNext =
			faceBounding.iterSegment[ixNextSeg]->first;

		// Add nodes to active edge and next edge
		int ixActiveNode =
			BisectSegment(iterSegActive, vecNodes);

		int ixNextNode =
			BisectSegment(
				faceBounding.iterSegment[ixNextSeg],
				vecNodes);

		// Add centroid node
		int ixCentroidNode =
			AddFaceCenterpointNode(faceBounding, vecNodes);
		...
		vecFaces.push_back(Face(
			faceBounding[ixActiveSeg],
			ixActiveNode,
			ixCentroidNode,
			faceBounding[(ixActiveSeg+3)%4]));

		vecFaces.push_back(Face(
			faceBounding[(ixActiveSeg+2)%4],
			faceBounding[(ixActiveSeg+3)%4],
			ixCentroidNode,
			ixNextNode));
