...
void DissectSegment(
	...
	if ((nCuts < 1) || (nCuts > 3)) {
		_EXCEPTIONT("Invalid number of cuts");
	}
	if ((iter->second.nMidpoints != 0) && (iter->second.nMidpoints != nCuts)) {
		_EXCEPTION2("\n\tSegment has already been dissected differently"
			"\n\t(Request %i : Current %i)",
			nCuts, iter->second.nMidpoints);
		return;
	}
	if ((nCuts != 0) && (iter->second.nMidpoints == nCuts)) {
		return;
	}
...
void MergeNodes(
	...
	if (ixNodeSource == ixNodeTarget) {
		_EXCEPTIONT("Invalid source and target node");
	}

	for (int i = 0; i < vecFaces.size(); i++) {
		for (int c = 0; c < 4; c++) {
			if (vecFaces[i][c] == ixNodeSource) {
				vecFaces[i][c] = ixNodeTarget;
			}
		}
	}

	// Merge coordinates
	vecNodes[ixNodeTarget].x = 0.5 * (
		+ vecNodes[ixNodeTarget].x
		+ vecNodes[ixNodeSource].x);
	vecNodes[ixNodeTarget].y = 0.5 * (
		+ vecNodes[ixNodeTarget].y
		+ vecNodes[ixNodeSource].y);
	vecNodes[ixNodeTarget].z = 0.5 * (
		+ vecNodes[ixNodeTarget].z
		+ vecNodes[ixNodeSource].z);

	double dRadius = sqrt(
		+ vecNodes[ixNodeTarget].x * vecNodes[ixNodeTarget].x
...
void RefineFace(
	...
	for (int k = 0; k < 4; k++) {
		if (faceOld.ixSplitMidpoints[k] != InvalidNode) {
			...
			ixMidpointNodes[k] =
				faceOld.iterSegment[k]->second.ixMidpointNode[0];

		} else if (faceOld.iterSegment[k]->second.nMidpoints == 3) {
			ixMidpointNodes[k] =
				faceOld.iterSegment[k]->second.ixMidpointNode[1];

			// Orientation of segment relative to face determines order
			// of split midpoints.
			if (faceOld.iterSegment[k]->first[0] == faceOld[k]) {
				ixSplitMidpoints[2*k] =
					faceOld.iterSegment[k]->second.ixMidpointNode[0];
				ixSplitMidpoints[2*k+1] =
					faceOld.iterSegment[k]->second.ixMidpointNode[2];
	...
	int ixCentroidNode =
		AddFaceCenterpointNode(faceOld, vecNodes);

	int nNewRefineLevel = vecFaces[ixFace].nRefineLevel;

	// Insert new faces by bisecting existing face into quarters
	Face faceSE(faceOld[0], ixMidpointNodes[0],
			ixCentroidNode, ixMidpointNodes[3], nNewRefineLevel);

	Face faceSW(ixMidpointNodes[0], faceOld[1],
			ixMidpointNodes[1], ixCentroidNode, nNewRefineLevel);

	Face faceNW(ixCentroidNode, ixMidpointNodes[1],
			faceOld[2], ixMidpointNodes[2], nNewRefineLevel);

	Face faceNE(ixMidpointNodes[3], ixCentroidNode,
			ixMidpointNodes[2], faceOld[3], nNewRefineLevel);
...
void DeleteTaggedFaces(
	...
	// Delete tagged faces
	int nOffset = 0;
	for (int i = 0; i + nOffset < vecFaces.size(); i++) {
...
void RefineGridLevel(
	...
	for (;;) {
		...
		for (int i = 0; i < nRefineSegments; i++) {
			...
			bool fFound = false;
			for (int j = 0; j < vecRefineSegments.size(); j++) {
			...
			if (!fFound) {
				_EXCEPTIONT("Cannot find connecting edge.");
			}

			// Found a complete loop
			if (ixNextNode == ixFirstNode) {
				break;
			}
		}
		...
		if (vecFaces[ixFace0].nRefineLevel > vecFaces[ixFace1].nRefineLevel) {
			nOrientation =
				vecFaces[ixFace0].SegmentOriention(
					vecOrderedSegments[0]->first);
		} else {
			nOrientation =
				vecFaces[ixFace1].SegmentOriention(
					vecOrderedSegments[0]->first);
		}
		...
		if (nOrientation == (-1)) {
			for (int i = 0; i < nOrderedSegmentsCount/2; i++) {
				...
				vecOrderedSegments[i] =
					vecOrderedSegments[nOrderedSegmentsCount-i-1];
				vecOrderedSegments[nOrderedSegmentsCount-i-1] =
					iterTemp;
			}
		...
		for (int i = 0; i < nOrderedSegmentsCount; i++) {
			...
			if (vecFaces[vecOrderedSegments[i]->second[0]].nRefineLevel <
				...
				vecFaces[vecOrderedSegments[i]->second[0]].nRefineLevel >
				vecFaces[vecOrderedSegments[i]->second[1]].nRefineLevel
			) {
...
void RefineGrid(
	...
	for (; iRefineLevel < refmap.GetMaxRefineLevel(); iRefineLevel++) {
		...
		RefineGridLevel(
			vecNodes, vecFaces, mapSegments, reftemp, iRefineLevel+1);
