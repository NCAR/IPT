...
void SpringDynamics(
				...
				if (vecFaces[n].nRefineLevel != (-1)) {
					for (int k = 0; k < 4; k++) {
						MobileNodeIterator iterNode =
							mapMobileNodes.find(vecFaces[n][k]);

						if (iterNode != mapMobileNodes.end()) {
							mapMobileNodes.erase(iterNode);
						}
					}
				}
			}
			...
			for (int z = 0; z < nTransSmoothDist-1; z++) {
				...
				for (; iterSeg != mapSegments.end(); iterSeg++) {
					...
					if (iterNode != mapOldMobileNodes.end()) {
						mapMobileNodes.insert(
							MobileNodePair(iterSeg->first[1], Node()));
					}

					iterNode = mapOldMobileNodes.find(iterSeg->first[1]);
					if (iterNode != mapOldMobileNodes.end()) {
						mapMobileNodes.insert(
							MobileNodePair(iterSeg->first[0], Node()));
					}
				}
			}
		}
	...
	printf("....Number of active nodes: %lu\n", mapMobileNodes.size());
	printf("....Number of active segments: %lu\n", mapSegments.size());
	...
	for (int i = 0; i < nSmoothIterations; i++) {
		...
		for (; iterSeg != mapSegments.end(); iterSeg++) {
			Node disp;
			disp.x =
				+ vecNodes[iterSeg->first[1]].x
				- vecNodes[iterSeg->first[0]].x;
			disp.y =
				+ vecNodes[iterSeg->first[1]].y
				- vecNodes[iterSeg->first[0]].y;
			disp.z =
				+ vecNodes[iterSeg->first[1]].z
				- vecNodes[iterSeg->first[0]].z;

			MobileNodeIterator iterNode0 =
				mapMobileNodes.find(iterSeg->first[0]);
			MobileNodeIterator iterNode1 =
				mapMobileNodes.find(iterSeg->first[1]);
...
*/
		...
		iterNode = mapMobileNodes.begin();
		for (; iterNode != mapMobileNodes.end(); iterNode++) {
			dTotalDisplacement += sqrt(
				+ iterNode->second.x * iterNode->second.x
