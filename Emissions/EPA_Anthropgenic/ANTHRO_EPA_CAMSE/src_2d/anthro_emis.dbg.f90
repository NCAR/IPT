     ...
     do src = 1,nsrc_names
       if( src_active(src) ) then
         ...
         if( data_file(src)%is_EPA_STACK ) then
           ...
           data_file(src)%stack%filespec = trim(anthro_dir) // '/' // trim(src_names(src)) // '/' // trim(data_file(src)%stack%filename)
         endif
...
!                      endif
      ...
      minndx(:) = minloc(anthro_map(1)%emissionSrf(:,:,:))
      maxndx(:) = maxloc(anthro_map(1)%emissionSrf(:,:,:))
      write(*,*) 'Min mdl i,j,k = ',minndx(:)
      write(*,*) 'Max mdl i,j,k = ',maxndx(:)
      write(*,*) 'Min mdl val = ',minval(anthro_map(1)%emissionSrf(:,:,:))
      write(*,*) 'Max mdl val = ',maxval(anthro_map(1)%emissionSrf(:,:,:))
!---------------------------------------------------------------------
!   write emission file
!---------------------------------------------------------------------
...
!---------------------------------------------------------------------
     ...
     model_grid%z_at_w(1,1:model_grid%nlevs+1,1) = (/ (50.*real(k),k=0,model_grid%nlevs) /)
...
!---------------------------------------------------------------------
        call area_interp( data_file, grid, grid%model_area_type, wrk_emis(:,:,1), diag_level )
      else is_area_map
!---------------------------------------------------------------------
...
!---------------------------------------------------------------------
      ...
      do stk = 1,data_file%stack%nStk
        if( data_file%stack%dataMask(stk) ) then
          if( data_file%stack%mdl_i(stk) == 0  .or. &
              data_file%stack%mdl_j(stk) == 0  .or. &
              data_file%stack%mdl_k(stk) == 0 ) then
            write(*,*) 'map_src_emissions: ERROR with stack i,j,k'
            stop 'ERROR'
          endif
        endif
      enddo
      write(*,*) 'active stk count = ',count(data_file%stack%dataMask(:))
      write(*,*) 'Min i val = ',minval(data_file%stack%mdl_i(:),mask=data_file%stack%dataMask(:) )
      write(*,*) 'Max i val = ',maxval(data_file%stack%mdl_i(:),mask=data_file%stack%dataMask(:) )
      write(*,*) 'Min j val = ',minval(data_file%stack%mdl_j(:),mask=data_file%stack%dataMask(:) )
      write(*,*) 'Max j val = ',maxval(data_file%stack%mdl_j(:),mask=data_file%stack%dataMask(:) )
      write(*,*) 'Min k val = ',minval(data_file%stack%mdl_k(:),mask=data_file%stack%dataMask(:) )
      write(*,*) 'Max k val = ',maxval(data_file%stack%mdl_k(:),mask=data_file%stack%dataMask(:) )
      write(*,*) 'Min src val = ',minval(data_file%stack%src_data(:),mask=data_file%stack%dataMask(:) )
      write(*,*) 'Max src val = ',maxval(data_file%stack%src_data(:),mask=data_file%stack%dataMask(:) )
      do stk = 1,data_file%stack%nStk
        if( data_file%stack%dataMask(stk) ) then
          il = data_file%stack%mdl_i(stk) ; jl = data_file%stack%mdl_j(stk)
          k = data_file%stack%mdl_k(stk)
          wrk_emis(il,jl,k) = data_file%stack%src_data(stk)
        endif
      enddo
      write(*,*) 'Min mdl val = ',minval(wrk_emis )
      write(*,*) 'Max mdl val = ',maxval(wrk_emis )
    endif not_STACK
...
!-----------------------------------------------------------------------
            ...
            message = 'write_cam_emis: Failed to define emis_anthro variable'
            call handle_ncerr( nf_def_var( ncid, 'emis_anthro', nf_float, n, dims, varid ), &
                               trim(message) )
!-----------------------------------------------------------------------
!   ... define variable attributes
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
            ...
            message = 'write_cam_emis: Failed to get time dimension id'
...
!-----------------------------------------------------------------------
          ...
          if( is_initial_time ) then
            ...
            if( mdl_is_CAMFV .and. file == 2 ) then
              ...
              call handle_ncerr( nf_put_var_real( ncid, varid, &
                                 (/ (50.*(real(k)+.5),k=0,model_grid%nlevs-1) /) ), &
          ...
          if( file == 1 ) then
            ...
            length(:4)    = (/ ide, jde, emissions_zdim_stag, 1 /)
            n = 4
          ...
          call handle_ncerr( nf_inq_varid( ncid, 'emis_anthro', varid ), trim(message) )
!---------------------------------------------------------------------
