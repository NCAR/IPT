...
!-----------------------------------------------------------------
     do ns = 1,nemis
       ...
       allocate( anthro_map(ns)%emission(ide,jde,emissions_zdim_stag),stat=astat )
       if( astat /= 0 ) then
                   ...
                   if( trim(anthro_map(ns)%src_var(ns1)) ==  trim(src_names(src)) ) then
                     if( anthro_map(ns)%cat_wght(nc,ns1) /= 0.) then
                       ...
                       if( .not. data_file(src)%is_EPA_STACK ) then
                         anthro_map(ns)%emission(:,:,1) = anthro_map(ns)%emission(:,:,1) &
                                                        + scale_factor*wrk_emis(:,:,1)
                       else
                         do lev = 1,emissions_zdim_stag
                           anthro_map(ns)%emission(:,:,lev) = anthro_map(ns)%emission(:,:,lev) &
...
!       wrk_emis(:,:,2:emissions_zdim_stag) = 0.
        wrk_emis(:,:,:)    = anthro_map(m)%emission(:,:,:)
        if( serial_output ) then
          start_ndx(4) = 1
...
is_first_time: &
        if( is_initial_time ) then
          ...
          outpname = 'emissions-EPA_' // trim(anthro_map(m)%emis_name) // '_vertical_c' // cdate // '.nc'
!-----------------------------------------------------------------------
!     	... create netcdf anthro emission file and enter define mode
!-----------------------------------------------------------------------
          message = 'write_emis: Failed to create ' // trim( outpname )
...
!-----------------------------------------------------------------------
          ...
          call handle_ncerr( nf_def_dim( ncid, 'altitude', model_grid%nlevs, zdim_id ), &
                             'write_emis: Failed to define altitude dimension' )
          call handle_ncerr( nf_def_dim( ncid, 'altitude_int', model_grid%nlevs+1, zdim_int_id ), &
                             'write_emis: Failed to define altitude interface dimension' )
!       call handle_ncerr( nf_def_dim( ncid, 'emissions_zdim_stag', emissions_zdim_stag, zdim_id ), &
!                          'write_emis: Failed to define emissions_zdim_stag dimension' )
          call handle_ncerr( nf_def_dim( ncid, 'time', nf_unlimited, time_id ), &
                             'write_emis: Failed to create Time dimension' )
...
!-----------------------------------------------------------------------
          dims(1) = time_id
          call handle_ncerr( nf_def_var( ncid, 'time', nf_int, 1, dims, varid ), &
                             'write_emis: Failed to define time variable' )
          dims(1) = lon_id
          call handle_ncerr( nf_def_var( ncid, 'lon', nf_float, 1, dims, varid ), &
                             'write_emis: Failed to define lon variable' )
          dims(1) = lat_id
          call handle_ncerr( nf_def_var( ncid, 'lat', nf_float, 1, dims, varid ), &
                             'write_emis: Failed to define lat variable' )

          dims(1) = zdim_id
          call handle_ncerr( nf_def_var( ncid, 'altitude', nf_float, 1, dims, varid ), &
                             'write_emis: Failed to define altitude variable' )

          dims(1) = zdim_int_id
          call handle_ncerr( nf_def_var( ncid, 'altitude_int', nf_float, 1, dims, varid ), &
                             'write_emis: Failed to define altitude_int variable' )

          dims(:) = (/ lon_id, lat_id, zdim_id, time_id /)
          message = 'write_emis: Failed to define emis_anthro variable'
          call handle_ncerr( nf_def_var( ncid, 'emis_anthro', nf_float, 4, dims, varid ), &
                             trim(message) )
!-----------------------------------------------------------------------
!   ... define variable attributes
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
          ...
          call handle_ncerr( nf_open( trim(outpname), nf_noclobber, ncid ), message )       
