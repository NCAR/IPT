       ...
       public :: diag_level, MaxChars, defaultUnits

       integer, parameter :: MaxChars = 256
       ...
       type species_map
          ...
          character(len=32) :: wrf_name                        ! mdl species name
          ...
          character(len=64) :: units = ' '                     ! mdl species units
          character(len=Maxchars) :: map                       ! map fire species -> mdl species
       end type species_map

       character(len=64) :: defaultUnits = 'molecules/cm^2/s'  ! mdl species default units
...
!------------------------------------------------------------------
      ...
      integer :: firecnt
      integer :: slen
      character(len=MaxChars) :: mstring
      logical :: one2one
...
!------------------------------------------------------------------
         ...
         wrf2fire_type(m)%map = trim(mstring)
         if( diag_level >= 200 ) then
           write(*,*) 'mapper: mstring = ',trim(mstring)
         endif
!------------------------------------------------------------------
!	... get the mdl species token
!------------------------------------------------------------------
         sl = index( mstring, '->' )
         if( sl < 1 ) then
           sl = index( mstring, '=' )
...
!------------------------------------------------------------------
!	... set the mdl species units if specified
!------------------------------------------------------------------
         sbl = index( mstring(:sl-1), '(' )
         if( sbl > 0 ) then
           sbu = index( mstring(:sl-1), ')', back=.true. )
           if( sbu > sbl ) then
             wrf2fire_type(m)%wrf_name = mstring(:sbl-1)
             wrf2fire_type(m)%units = mstring(sbl+1:sbu-1)
             wrf2fire_type(m)%map = mstring(1:sbl-1) // mstring(sbu+1:)
           endif
         else
           wrf2fire_type(m)%wrf_name = mstring(:sl-1)
           wrf2fire_type(m)%units = trim( defaultUnits )
         end if 
         firecnt = 1
         sr = sl + 2
         sl = sr
