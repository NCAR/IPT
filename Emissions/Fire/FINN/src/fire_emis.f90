...
!-----------------------------------------------------------------
   ...
   character(len=maxsize) :: message
   character(len=linsize) :: filespec
   character(len=namsize) :: mdlFilenm
   character(len=namsize) :: mdlDir
   character(len=1024)    :: buffer
   character(len=32)      :: dummy(16)
   character(len=8)       :: Version(max_files)
   ...
   type(model_grid_type) :: mdlGrd
   type(UtilAttributes ) :: UtilAttrs
...
!-----------------------------------------------------------------
   ...
   character(len=MaxChars) :: wrf2fire_map(map_size) = ' '
   character(len=MaxChars) :: glb2fire_map(map_size) = ' '
   ...
   namelist /control/ domains, fire_directory, wrf_directory, fire_filename, &
                      ...
                      avrg_fire_size, max_fire_size, diag_level, resol, Model, &
                      output_timing, glb2fire_map, mdlDir, mdlFilenm, defaultUnits, &
                      FinnVers, EmisType
...
!-------------------------------------------------------------------------
   ...
   is_wrf   = trim( Model ) == 'WRF'
   is_CAMSE = trim( Model ) == 'CAMSE'
   isnt_CAMSE = .not. is_CAMSE
   mdlGrd%mdl_is_CAMSE = is_CAMSE
   is_glb = .not. is_wrf
!-----------------------------------------------------------------
...
!-----------------------------------------------------------------
!     form model to fire emission map
!-----------------------------------------------------------------
   ...
   if( is_glb  ) then
     ...
     UtilAttrs%Model = Model
     UtilAttrs%resol = resol
     UtilAttrs%EmisType = EmisType
     UtilAttrs%FinnVers = FinnVers
     if( isnt_CAMSE ) then
       call glb_file( UtilAttrs, proj(1), n_wrf_spc, n_total_days, start_date, end_date, &
                      n_infiles, fire_directory, fire_filename, output_timing, n_mnths, mdlGrd )
     endif
   endif
...
!-------------------------------------------------------------------------
     ...
     write(*,*) 'fire_emis: this usually takes about 30 - 45 seconds'
     call start_timing
     nlines = get_file_size( 10 ) - 1
     call end_timing( 'Line count' )
...
!  set FINN version
!-------------------------------------------------------------------------
     if( index( trim(buffer),'POLYID' ) /= 0 .and. &
         index( trim(buffer),'FIREID' ) /= 0 ) then
       Version(file) = '2.0'
     else
       Version(file) = '1.5'
     endif
...
!-------------------------------------------------------------------------
     ...
     n_fire_spc = count(gen_ndx(:) == comma) - nSkip
     deallocate( gen_ndx )

     write(*,*) ' '
     write(*,'(''fire_emis: there are '',i3,'' source species in the fire file'')') n_fire_spc
     ...
     read(unit=10,fmt=*,iostat=istat) dummy(1:nSkip),fire_species(1:n_fire_spc)
     if( istat /= 0 ) then
...
!-------------------------------------------------------------------------
!  synch FinnVers with actual version
!-------------------------------------------------------------------------
   if( all(Version(1:n_infiles) == '1.5') ) then
     FinnVers = '1.5'
   elseif( all(Version(1:n_infiles) == '2.0') ) then
     FinnVers = '2.0'
   else
     FinnVers = '1.5&2.0'
   endif
...
!-------------------------------------------------------------------------
     ...
     close( 10 )
     call end_timing( 'Read FINN file' )
!-------------------------------------------------------------------------
!  check days
!-------------------------------------------------------------------------
...
file_loop_b : &
   ...
   elseif( is_CAMSE ) then
     filespec = trim( mdlDir ) // '/' // trim( mdlFilenm )
!---------------------------------------------------------------------
!   open model input file
!---------------------------------------------------------------------
     message = 'fire_emis: Failed to open ' // trim(filespec)
     call handle_ncerr( nf_open( trim(filespec), nf_noclobber, ncid ), message )       

     call start_timing
     call camse_init( ncid, mdlGrd )
     call end_timing( 'camse_init' )
     call glb_file( UtilAttrs, proj(1), n_wrf_spc, n_total_days, start_date, end_date, &
                    n_infiles, fire_directory, fire_filename, output_timing, n_mnths, mdlGrd )
!---------------------------------------------------------------------
!   close model file
!---------------------------------------------------------------------
     message = 'model_init: Failed to close ' // trim(filespec)
...
days_loop: &
       do day_ndx = 1,day_cnt-1
         ...
         write(*,*) ' '
         write(*,'(''fire_emis: processing '',i6,'' fires on day '',i3)') m2-m1+1,day_ndx
         ...
         do m = m1,m2
           if( has_fire_size(file) ) then
             select case( Version(file) )
               ...
               case( '2.0' )
                 read(unit=10,fmt=*,iostat=istat) timeod,timeod,timeod,genveg,lat(m),lon(m),fire_size(m),fire_emissions(:,m)
             end select
           else
             select case( Version(file) )
               case( '1.5' )
               ...
               case( '2.0' )
                 read(unit=10,fmt=*,iostat=istat) timeod,timeod,timeod,genveg,lat(m),lon(m),fire_emissions(:,m)
             end select
           ...
           if( is_glb .and. lon(m) < 0. ) then
             lon(m) = mod( lon(m)+THREE60,THREE60 )
           endif
is_not_CAMSE: &
           if( isnt_CAMSE ) then
         ...
         if( is_CAMSE ) then
             call start_timing
             call camse_mapper( lon, lat, mdlGrd, lon_ndx )
             call end_timing( 'camse_mapper' )
         endif
         if( is_wrf ) then
           call write_fire_file( domain, wrk_date, n_wrf_spc, m1, m2, &
                                 ...
                                 fire_filename, file, dom_glb_attrs(domain)%ngatts, &
                                 dom_glb_attrs(domain)%attrs, FinnVers )
         else
