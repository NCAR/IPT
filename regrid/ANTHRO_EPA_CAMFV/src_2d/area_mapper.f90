   ...
   public :: ll_2_ij
   public :: lm_2_xy, xy_n_quad, pnt_in_quad
...
!-------------------------------------------------------------
   ...
   subroutine area_interp_init( mdl_is_CAMFV, grid, model_area_type, diag_level )
!-------------------------------------------------------------
...
!-------------------------------------------------------------
   ...
   logical, intent(in) :: mdl_is_CAMFV
...
!-------------------------------------------------------------
   ...
   real    :: dataCellArea(grid%nlons,grid%nlats)
...
!-------------------------------------------------------------
   ...
   if( astat /= 0 ) then
     write(*,*) 'area_interp; failed to allocate dcell_msk: error = ',astat
     stop
   endif
   ...
   if( astat /= 0 ) then
     write(*,*) 'area_interp; failed to allocate xedge_2d_ij: error = ',astat
     stop
   endif
   ...
   if( astat /= 0 ) then
     write(*,*) 'area_interp; failed to allocate yedge_2d_ij: error = ',astat
     stop
   endif
...
!-------------------------------------------------------------
   do j = 1,nlat+1
     do i = 1,nlon+1
       call llij( real( grid%yedge_2d(i,j),kind=4 ), real( grid%xedge_2d(i,j),kind=4 ), proj, &
...
mcell_has_dcells: &
             ...
             if( dcell_msk(i,j) ) then
               ...
               if( mdl_is_CAMFV ) then
                 model_area_type(mlon_ndx,mlat_ndx)%wght(dcell_ndx) = &
                   abs(real(quadintersectarea( Quad1_x, Quad1_y, Quad2_x, Quad2_y),kind=4)) &
                   / dataCellArea(i,j)
               else
                 model_area_type(mlon_ndx,mlat_ndx)%wght(dcell_ndx) = &
                   abs(real(quadintersectarea( Quad1_x, Quad1_y, Quad2_x, Quad2_y),kind=4))
   ...
   grid%xedge_2d_ij(:,:) = xedge_2d_ij(:,:)
   grid%yedge_2d_ij(:,:) = yedge_2d_ij(:,:)
...
!-------------------------------------------------------------
   ...
   wrk_emis(:,:) = 0.
   m_lat_s = 1 ; m_lat_e = proj%jydim-1
   m_lon_s = 1 ; m_lon_e = proj%ixdim-1
!-------------------------------------------------------------
!  loop over model cells
!-------------------------------------------------------------
model_lat_loop : &
...
!---------------------------------------------------------------------
   ...
   logical function lines_overlap( x1, x2, y1, y2 )
...
!---------------------------------------------------------------------
   ...
   subroutine set_alphabeta( xv, yv, alpha, beta )
!---------------------------------------------------------------------
!  set alpha, beta coefficients
!---------------------------------------------------------------------

   real, intent(in)  :: xv(4), yv(4)
   real, intent(out) :: alpha(4), beta(4)

   alpha(1) = xv(1)
   alpha(2) = xv(2) - xv(1)
   alpha(3) = xv(4) - xv(1)
   alpha(4) = xv(1) + xv(3) - (xv(2) + xv(4))

   beta(1) = yv(1)
   beta(2) = yv(2) - yv(1)
   beta(3) = yv(4) - yv(1)
   beta(4) = yv(1) + yv(3) - (yv(2) + yv(4))

   end subroutine set_alphabeta

   subroutine lm_2_xy( xv, yv, lm, xy )
!---------------------------------------------------------------------
...
!---------------------------------------------------------------------
   ...
   call set_alphabeta( xv, yv, alpha, beta )

   wrk = lm(1)*lm(2)
   xy(1) = alpha(1) + alpha(2)*lm(1) + alpha(3)*lm(2) + alpha(4)*wrk
   xy(2) = beta(1)  + beta(2)*lm(1)  + beta(3)*lm(2)  + beta(4)*wrk

   end subroutine lm_2_xy

   subroutine xy_2_lm( xv, yv, xy, lm )
!---------------------------------------------------------------------
...
!---------------------------------------------------------------------
   ...
   bb = alpha(4)*beta(1) + alpha(2)*beta(3) + xy(1)*beta(4) &
        - (alpha(1)*beta(4) + alpha(3)*beta(2) + xy(2)*alpha(4))
   cc = alpha(2)*beta(1) + xy(1)*beta(2) - (alpha(1)*beta(2) + xy(2)*alpha(2))

   lm(2) = .5*(sqrt(bb*bb - 4.*aa*cc) - bb)/aa
   lm(1) = (xy(1) -(alpha(1) + alpha(3)*lm(2)))/(alpha(2) + alpha(4)*lm(2))

   end subroutine xy_2_lm

   subroutine set_wghts( xv, yv, xy, wghts )
!---------------------------------------------------------------------
!  midpoint of general quadrilateral
!---------------------------------------------------------------------
   ...
   call xy_2_lm( xv, yv, xy, lm )

   wghts(:) = (/ lm(1), 1. - lm(1), lm(2), 1. - lm(2) /)

   end subroutine set_wghts

   logical function xy_n_quad( xv, yv, xy )
!---------------------------------------------------------------------
!  midpoint of general quadrilateral
!---------------------------------------------------------------------
   ...
   if( xy_n_quad ) then
     l = (xy(1) -(alpha(1) + alpha(3)*m))/(alpha(2) + alpha(4)*m)
